...
def setup_logging(module_name: str, log_file: str, log_level: str = 'DEBUG', **kwargs) -> Logger:
    """
    ...
    Args:
        module_name (str): Tên module (tên logger).
        log_file (str): Đường dẫn đến tệp log.
        log_level (str, optional): Mức log (DEBUG, INFO, WARN, ERROR...). Mặc định là 'DEBUG'.
    ...
    logger = logging.getLogger(module_name)
    # Lấy log_level an toàn bằng getattr
    safe_log_level = getattr(logging, log_level.upper(), logging.DEBUG)
    logger.setLevel(safe_log_level)
