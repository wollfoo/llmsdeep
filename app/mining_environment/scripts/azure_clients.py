...
class AzureBaseClient:
    ...
    def discover_resources(self, resource_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Đã khám phá {len(resources)} tài nguyên loại {resource_type}.")
            return resources
        except Exception as e:
            logger.error(f"Lỗi khi khám phá tài nguyên loại {resource_type}: {e}")
            return []
...
class AzureMonitorClient(AzureBaseClient):
    ...
    def get_metrics(self, resource_id: str, metric_names: List[str], timespan: str = 'PT1H', interval: str = 'PT1M') -> Dict[str, List[float]]:
        ...
        try:
            ...
            logger.info(f"Đã lấy metrics cho tài nguyên {resource_id}: {metric_names}")
            return metrics
        except Exception as e:
            logger.error(f"Lỗi khi lấy metrics từ Azure Monitor cho tài nguyên {resource_id}: {e}")
            return {}
...
class AzureSentinelClient(AzureBaseClient):
    ...
    def get_recent_alerts(self, days: int = 1) -> List[Any]:
        ...
        try:
            ...
            logger.info(f"Đã lấy {len(recent_alerts)} alerts từ Azure Sentinel trong {days} ngày gần đây.")
    ...
    def get_workspace_id(self) -> str:
        ...
        try:
            ...
            if resources:
                ...
                logger.info(f"Đã tìm thấy Log Analytics Workspace: {workspace['name']}")
                return workspace['id'].split('/')[-1]
            else:
                logger.warning("Không tìm thấy Log Analytics Workspace nào.")
                return ""
        except Exception as e:
            logger.error(f"Lỗi khi lấy Workspace ID: {e}")
            return ""
...
class AzureSecurityCenterClient(AzureBaseClient):
    ...
    def get_security_recommendations(self) -> List[Any]:
        ...
        try:
            ...
            logger.info(f"Đã lấy {len(recommendations_list)} security recommendations từ Azure Security Center.")
...
class AzureNetworkWatcherClient(AzureBaseClient):
    ...
    def get_flow_logs(self, resource_group: str, network_watcher_name: str, nsg_name: str) -> List[Any]:
        ...
        try:
            ...
            logger.info(f"Đã lấy {len(flow_logs)} flow logs từ NSG {nsg_name} trong Resource Group {resource_group}.")
            return flow_logs
        except Exception as e:
            logger.error(f"Lỗi khi lấy flow logs từ NSG {nsg_name} trong Resource Group {resource_group}: {e}")
            return []
...
class AzureTrafficAnalyticsClient(AzureBaseClient):
    ...
    def get_traffic_workspace_id(self) -> str:
        ...
        try:
            ...
            if resources:
                workspace = resources[0]
                logger.info(f"Đã tìm thấy Traffic Analytics Workspace: {workspace['name']}")
                return workspace['id'].split('/')[-1]
            else:
                logger.warning("Không tìm thấy Traffic Analytics Workspace nào.")
                return ""
        except Exception as e:
            logger.error(f"Lỗi khi lấy Traffic Analytics Workspace ID: {e}")
            return ""
    ...
    def get_traffic_data(self) -> List[Any]:
        ...
        """
        if not self.workspace_id:
            logger.error("Không có Traffic Analytics Workspace ID để lấy dữ liệu.")
            return []
        try:
            query = """
            AzureDiagnostics
            ...
            response = self.client.query(workspace_id=self.workspace_id, body=body)
            logger.info("Đã lấy dữ liệu Traffic Analytics thành công.")
            return response.tables
        except Exception as e:
            logger.error(f"Lỗi khi lấy dữ liệu Traffic Analytics: {e}")
            return []
...
class AzureMLClient(AzureBaseClient):
    ...
    def discover_ml_clusters(self) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Đã khám phá {len(clusters)} Azure ML Clusters.")
        return clusters
